...
int AUList::getMin(){
	int smallest = ListItems[0];
	for (int i = 0; i < length; ++i){
		if(ListItems[i] < smallest){
			smallest = ListItems[i];
		}
	}
	return smallest;
}
...
int AUList::getRange(){
	int small = ListItems[0];
	int large = ListItems[0];
	for(int loc = 0; loc < length; loc++){
		if(ListItems[loc] < small){
			small = ListItems[loc];
		}
		if(ListItems[loc] > large){
			large = ListItems[loc];
		}
	}
	return large-small;
...
AUList AUList::DuplicateSE(int first, int last) {
	...
	for(int i = first; i <= last; i++){
		duplicates.PutItem(ListItems[i]);
	}
