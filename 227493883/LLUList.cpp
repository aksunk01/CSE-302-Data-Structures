...
int LLUList::getRange() const{
	int min_value = ListStart->item;
	int max_value = ListStart->item;
	
	LNode* node = ListStart->next;
	while(node != nullptr){
		min_value = std::min(min_value, node->item);
		max_value = std::max(max_value, node->item);
		node = node->next;
	}
