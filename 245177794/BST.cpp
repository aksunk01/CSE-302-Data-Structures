...
int BST::getHeight(TNode* curNode) const{
	if(curNode = NULL){
		return -1;
...
void BST::SplitBalance(){
	...
	inOrderToArray(root, sortedArray, arrayIndex);
	
	MakeEmpty();
	root = NULL;
	
	TNode* newRoot = buildBalanceTree(sortedArray, 0, arrayIndex - 1);
	root = newRoot;
}


void BST::inOrderToArray(TNode* curNode, int sortedArray[], int& arrayIndex) const{
	if(curNode != NULL){
		inOrderToArray(curNode->left, sortedArray, arrayIndex);
		sortedArray[arrayIndex++] = curNode ->item;
		inOrderToArray(curNode->right, sortedArray, arrayIndex);
	}
}

TNode* BST::buildBalanceTree(int sortedArray[], int start, int end) {
	if(start > end){
		return NULL;
	}
	...
	newNode->item = sortedArray[mid];
	
	newNode->left = buildBalanceTree(sortedArray, start, mid-1);
	newNode->right = buildBalanceTree(sortedArray, mid+1, end);
	return newNode;
}